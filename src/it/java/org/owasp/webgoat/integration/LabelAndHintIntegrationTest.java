...
public class LabelAndHintIntegrationTest extends IntegrationTest {
  ...
  private Properties getProperties(String lang) {
    ...
    try {
      ...
      java.nio.file.Path baseDir = java.nio.file.Paths.get("src/main/resources/i18n").toRealPath().normalize();
      // Construct the requested file path
      java.nio.file.Path requestedPath = baseDir.resolve("messages" + lang + ".properties").normalize();
      // Check if requestedPath is within baseDir
      if (!requestedPath.startsWith(baseDir)) {
        throw new SecurityException("Invalid language parameter resulting in path traversal");
      }
      try (InputStream input = java.nio.file.Files.newInputStream(requestedPath)) {
        prop = new Properties();
        prop.load(input);
      }
    } catch (Exception e) {
