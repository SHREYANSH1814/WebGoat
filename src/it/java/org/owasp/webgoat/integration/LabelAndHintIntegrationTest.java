...
public class LabelAndHintIntegrationTest extends IntegrationTest {
  ...
  private Properties getProperties(String lang) {
    ...
    try {
      // Construct the path safely to prevent path traversal
      java.nio.file.Path basePath = java.nio.file.Paths.get("src/main/resources/i18n");
      java.nio.file.Path filePath = basePath.resolve("messages" + lang + ".properties").normalize();
      if (!filePath.startsWith(basePath)) {
        throw new SecurityException("Invalid language parameter: path traversal attempt detected");
      }
