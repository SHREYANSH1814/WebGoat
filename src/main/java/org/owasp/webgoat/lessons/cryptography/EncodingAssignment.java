...
public class EncodingAssignment implements AssignmentEndpoint {
  ...
  private String sanitizeBasicAuth(String input) {
    // Basic sanitization: allow only Base64 characters and colon
    if (input == null) {
      return null;
    }
    if (!input.matches("[A-Za-z0-9+/=]+")) {
      throw new IllegalArgumentException("Invalid characters in basicAuth");
    }
    return input;
  }
  ...
  public String getBasicAuth(HttpServletRequest request) {
    ...
    if (basicAuth == null) {
      String password =
          HashingAssignment.SECRETS[new Random().nextInt(HashingAssignment.SECRETS.length)];
      basicAuth = getBasicAuth(username, password);
      String sanitizedBasicAuth = sanitizeBasicAuth(basicAuth);
