...
public class EncodingAssignment implements AssignmentEndpoint {
  ...
  public String getBasicAuth(HttpServletRequest request) {
    ...
    if (basicAuth == null) {
      String password =
          HashingAssignment.SECRETS[new Random().nextInt(HashingAssignment.SECRETS.length)];
      String generatedBasicAuth = getBasicAuth(username, password);
      // Validate the generated basicAuth before storing it in session
      if (generatedBasicAuth != null && !generatedBasicAuth.isEmpty()) {
