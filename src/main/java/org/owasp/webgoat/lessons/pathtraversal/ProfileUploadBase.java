...
public class ProfileUploadBase implements AssignmentEndpoint {
  ...
  protected File cleanupAndCreateDirectoryForUser(String username) {
    ...
    var uploadDirectory = new File(baseDir.getCanonicalFile(), username).getCanonicalFile();
    if (!uploadDirectory.getPath().startsWith(baseDir.getCanonicalPath() + File.separator)) {
        throw new IOException("Invalid username directory path");
    }
