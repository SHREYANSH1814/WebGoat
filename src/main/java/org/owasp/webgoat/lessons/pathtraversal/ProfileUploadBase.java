...
public class ProfileUploadBase implements AssignmentEndpoint {
  ...
  protected File cleanupAndCreateDirectoryForUser(String username) {
    var baseDirectory = new File(this.webGoatHomeDirectory, "/PathTraversal/");
        var uploadDirectory = new File(baseDirectory, username).getCanonicalFile();
        if (!uploadDirectory.getPath().startsWith(baseDirectory.getCanonicalPath() + File.separator)) {
