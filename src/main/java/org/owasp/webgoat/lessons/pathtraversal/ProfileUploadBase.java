  ...
  protected byte[] getProfilePictureAsBase64(String username) {
    ...
    if (profileDirectoryFiles != null && profileDirectoryFiles.length > 0) {
      return Arrays.stream(profileDirectoryFiles)
          .filter(file -> {
            try {
              String canonicalDir = profilePictureDirectory.getCanonicalPath();
              String canonicalFile = file.getCanonicalPath();
              return FilenameUtils.isExtension(file.getName(), List.of("jpg", "png")) && canonicalFile.startsWith(canonicalDir + File.separator);
            } catch (IOException e) {
              return false;
            }
          })
