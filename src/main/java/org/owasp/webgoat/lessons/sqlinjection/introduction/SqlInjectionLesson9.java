...
public class SqlInjectionLesson9 implements AssignmentEndpoint {
  ...
  protected AttackResult injectableQueryIntegrity(String name, String auth_tan) {
    ...
    try (Connection connection = dataSource.getConnection()) {
      ...
      // do injectable query
      SqlInjectionLesson8.log(connection, queryInjection);
      java.sql.PreparedStatement preparedStatement = connection.prepareStatement("SELECT * FROM employees WHERE last_name = ? AND auth_tan = ?", TYPE_SCROLL_SENSITIVE, CONCUR_UPDATABLE);
