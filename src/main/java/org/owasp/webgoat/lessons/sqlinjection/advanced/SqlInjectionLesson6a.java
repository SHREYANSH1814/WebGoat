    ...
    try (ResultSet results = preparedStatement.executeQuery()) {
      ...
      if (!((results != null) && results.first())) {
        return failed(this)
            ...
            .output(YOUR_QUERY_WAS + query.replace("?", "'" + accountName + "'"))
            .build();
      }
      ...
      return verifySqlInjection(output, appendingWhenSucceded, query.replace("?", "'" + accountName + "'"));
    } catch (SQLException sqle) {
      return failed(this).output(sqle.getMessage() + YOUR_QUERY_WAS + query.replace("?", "'" + accountName + "'")).build();
    }
  }
