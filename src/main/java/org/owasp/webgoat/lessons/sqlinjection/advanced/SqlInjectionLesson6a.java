...
public class SqlInjectionLesson6a implements AssignmentEndpoint {
  ...
  private AttackResult executeSqlInjection(Connection connection, String query, boolean usedUnion) {
      String preparedQuery = "SELECT * FROM user_data WHERE last_name = ?";
      try (java.sql.PreparedStatement preparedStatement = connection.prepareStatement(preparedQuery, ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY)) {
        preparedStatement.setString(1, query.substring(query.indexOf("'") + 1, query.lastIndexOf("'")));
