...
public class SqlInjectionLesson4 implements AssignmentEndpoint {
  ...
  protected AttackResult injectableQuery(String query) {
      try (Connection connection = dataSource.getConnection()) {
        String sql = "";
        if (query != null && query.matches("[a-zA-Z0-9_ ]+")) {
          sql = query;
        } else {
          return failed(this).output("Invalid input detected").build();
        }
        try (java.sql.PreparedStatement statement = connection.prepareStatement(sql)) {
          statement.executeUpdate();
        connection.commit();
        try (Statement stmt = connection.createStatement(TYPE_SCROLL_INSENSITIVE, CONCUR_READ_ONLY)) {
          ResultSet results = stmt.executeQuery("SELECT phone from employees;");
