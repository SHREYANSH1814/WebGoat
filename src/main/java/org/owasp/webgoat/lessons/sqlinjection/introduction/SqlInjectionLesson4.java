...
public class SqlInjectionLesson4 implements AssignmentEndpoint {
  ...
  protected AttackResult injectableQuery(String query) {
      try (Connection connection = dataSource.getConnection()) {
        String sql = "SELECT phone from employees;";
        try (java.sql.PreparedStatement preparedStatement = connection.prepareStatement(sql, TYPE_SCROLL_INSENSITIVE, CONCUR_READ_ONLY)) {
        ...
        if (results.first()) {
          output.append("<span class='feedback-positive'>" + sql + "</span>");
